...
def sorted_reg_dict() -> Dict[str, int]:
    return sorted(config_regs, key=config_regs.get)

def print_reg_states(apds: APDS9960) -> None:
    buf2 = bytearray(2)
    for key in sorted(config_regs, key=config_regs.get):
        reg_val = _read8(apds.i2c_device, buf2, config_regs[key])
        print(" {:22} 0x{:02X} | 0x{:02X} | {:08b}".format(key, config_regs[key], reg_val, reg_val))

def _read8(i2c_device: I2CDevice, buf: bytearray, addr: int) -> int:
    buf[0] = addr
