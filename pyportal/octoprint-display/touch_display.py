...
SIZE_CURSOR_DEFAULT = const(10)
DIMMER_CURSOR_DELAY = const(100)  # Cursor dim delay in ms
DIMMER_BACKLIGHT_DELAY = const(600000)  # Backlight delay in ms
...
class TouchDisplay():
    ...
    def init_touchscreen(self, portal = None):
        ...
        if self._cursor_color:
            self._cursor_palette = displayio.Palette(3)
            self._cursor_palette.make_transparent(0x000000)
            self._cursor_palette[0] = COLOR_ZERO
            self._cursor_palette[1] = self._cursor_color
            self._channels = self.get_channels(self._cursor_palette[1])
            self._cursor = Circle(
                x0=self._HEIGHT//2, y0=self._HEIGHT//2, r=SIZE_CURSOR_DEFAULT,
                fill=self._cursor_palette[0], outline=self._cursor_palette[1], stroke=0)
            self._cursor.hidden = True
    ...
    def get_touch(self):
        ...
        if self._last_touch_val:
            if self._cursor.hidden:
                self._first_touch = True
            else:
                self._first_touch = False

            if self._disp.brightness == 0:
                self._wakeup_touch = True
            else:
                self._wakeup_touch = False
            ...
            if self._cursor:
                ...
                ### Reset cursor color ###
                self._cursor.fill = self._cursor_palette[1]
    ...
    def handle_dimmer(self):
        ...
        if self._touch_threshold_met:
            ...
            if self._cursor.fill > COLOR_ZERO:
                if self._cursor.fill > COLOR_THRESHOLD_GRAY:
                    # dim1: Desaturate to gray
                    self._channels = self.get_channels(self._cursor.fill)
                    ...
                    if self._cursor.fill - self._dimmer_shift < 0:
                        self._cursor.fill = COLOR_THRESHOLD_GRAY
                    else:
                        self._cursor.fill -= self._dimmer_shift
                elif self._cursor.fill > COLOR_ONE:
                    # dim1: Gray to black
                    self._cursor.fill -= COLOR_ONE
                elif self._cursor.fill <= COLOR_ONE:
                    # dim3: Final flip to black
                    self._cursor.fill = COLOR_ZERO
                    self._cursor.hidden = True
            else:
